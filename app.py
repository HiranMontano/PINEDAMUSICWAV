...
# Ocultar el chrome de Streamlit
st.markdown(
    "<style>"
...
# HTML original completo — se pasa íntegro a components.html()
# (único cambio: se corrigen los backticks del template literal de YouTube
#  que estaban rotos en el HTML original, y se agrega un script de
#  auto-resize para que el iframe se ajuste a la altura real del contenido)
HTML = """<!DOCTYPE html>
<html lang="es">
<head>
  <meta charset="UTF-8" />
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .navlinks a { padding: .45rem .1rem; color: rgba(240,240,246,.76); border-bottom: 2px solid transparent; transition: .2s ease; }
    .navlinks a:hover { color: var(--paper); border-color: var(--red); }
    .section { padding: clamp(4rem, 9vw, 7.5rem) clamp(1rem, 4vw, 3rem); }
    .section-kicker { display: inline-block; margin-bottom: 1rem; color: var(--red); font-weight: 700; text-transform: uppercase; letter-spacing: .22em; font-size: .72rem; }
    .section-title { font-family: 'Archivo Black', sans-serif; text-transform: uppercase; font-size: clamp(2.2rem, 7vw, 6rem); line-height: .86; letter-spacing: -0.08em; max-width: 12ch; text-shadow: 4px 4px 0 #000; }
    .red-word { color: var(--red); }
    .button-row { display: flex; flex-wrap: wrap; gap: .85rem; margin-top: 1.75rem; }
    .btn { display: inline-flex; align-items: center; justify-content: center; min-height: 44px; padding: .85rem 1rem; border: 2px solid var(--paper); box-shadow: 5px 5px 0 #000; background: var(--red); color: var(--paper); font-family: 'Archivo Black', sans-serif; text-transform: uppercase; letter-spacing: .04em; transition: transform .18s ease, box-shadow .18s ease, background .18s ease; }
    ...
    .stamp { display: inline-flex; align-items: center; gap: .55rem; transform: rotate(-2deg); padding: .35rem .6rem; border: 2px solid var(--red); color: var(--red); font-size: .72rem; font-weight: 700; text-transform: uppercase; letter-spacing: .18em; margin-bottom: 1.2rem; background: rgba(0,0,0,.22); }
    .bio-headline { font-family: 'Archivo Black', sans-serif; font-size: clamp(3rem, 10vw, 8.5rem); line-height: .78; letter-spacing: -0.09em; text-transform: uppercase; text-shadow: 6px 6px 0 #000; }
    .bio-headline .glitch { display: inline-block; font-family: 'Anton', 'Archivo Black', sans-serif; color: var(--red); letter-spacing: -0.04em; text-shadow: 5px 5px 0 #000; filter: none; opacity: 1; line-height: .82; transform: translateY(2px); -webkit-font-smoothing: antialiased; text-rendering: geometricPrecision; }
    .bio-text { margin-top: 1.6rem; max-width: 680px; font-size: clamp(1rem, 1.7vw, 1.25rem); line-height: 1.8; color: rgba(240,240,246,.82); }
    .bio-text strong { color: var(--paper); }
    .role-tags { display: flex; flex-wrap: wrap; gap: .65rem; margin-top: 1.3rem; }
    .tag { padding: .42rem .62rem; background: rgba(240,240,246,.08); border: 1px solid rgba(240,240,246,.18); color: rgba(240,240,246,.78); text-transform: uppercase; font-size: .72rem; letter-spacing: .12em; }
    .bio-card { position: relative; min-height: 520px; border: 2px solid var(--paper); background: linear-gradient(0deg, rgba(236,13,13,.25), rgba(0,0,0,.1)), linear-gradient(135deg, var(--brick), var(--charcoal)); box-shadow: 18px 18px 0 #000; overflow: hidden; transform: rotate(1.2deg); }
    .bio-card::before { content: none; }
    .bio-main-img { width: 100%; height: 100%; object-fit: cover; object-position: center; filter: contrast(1.08) saturate(0.95); transition: transform .35s ease, filter .35s ease; }
    .bio-card:hover .bio-main-img { transform: scale(1.04); filter: contrast(1.15) saturate(1.05); }
    .parental { position: absolute; right: 1rem; bottom: 1rem; padding: .35rem .45rem; border: 2px solid var(--paper); color: var(--paper); background: #000; font-family: Arial, sans-serif; font-weight: 900; font-size: .75rem; line-height: .85; text-align: center; text-transform: uppercase; max-width: 110px; }
    .image-grid { display: grid; grid-template-columns: repeat(12, 1fr); gap: .9rem; margin-top: 2.5rem; }
    .image-card { min-height: 260px; border: 2px solid rgba(240,240,246,.75); background: linear-gradient(135deg, rgba(236,13,13,.28), rgba(105,57,71,.5)), var(--charcoal); position: relative; overflow: hidden; box-shadow: 8px 8px 0 #000; }
    ...
    .image-card::after { content: attr(data-label); position: absolute; left: .85rem; bottom: .85rem; background: var(--paper); color: #000; font-family: 'Archivo Black', sans-serif; font-size: clamp(1.2rem, 3vw, 2.5rem); text-transform: uppercase; line-height: .9; padding: .25rem .45rem; box-decoration-break: clone; -webkit-box-decoration-break: clone; }
    .portfolio-img { width: 100%; height: 100%; object-fit: cover; object-position: center; filter: contrast(1.08) saturate(0.9); transition: transform .35s ease, filter .35s ease; }
    .image-card:hover .portfolio-img { transform: scale(1.06); filter: contrast(1.15) saturate(1.05); }
    .video-layout { display: grid; grid-template-columns: 1.4fr .9fr; gap: 1rem; margin-top: 2.5rem; }
    .video-card { position: relative; min-height: 280px; display: grid; place-items: center; border: 2px solid rgba(240,240,246,.8); background: linear-gradient(135deg, rgba(0,0,0,.55), rgba(236,13,13,.18)), repeating-linear-gradient(0deg, #1b1820 0 6px, #211d25 6px 12px); overflow: hidden; box-shadow: 8px 8px 0 #000; color: var(--paper); text-decoration: none; }
    .video-main { min-height: 600px; }
    .video-stack { display: grid; gap: 1rem; }
    .video-thumb { position: absolute; inset: 0; width: 100%; height: 100%; max-width: none; object-fit: cover; object-position: center; z-index: 0; filter: contrast(1.08) saturate(0.9) brightness(0.68); transition: transform .35s ease, filter .35s ease; }
    .video-card:hover .video-thumb { transform: scale(1.06); filter: contrast(1.15) saturate(1.05) brightness(0.82); }
    .video-card::after { content: ""; position: absolute; inset: 0; z-index: 1; background: linear-gradient(to top, rgba(0,0,0,.92) 0%, rgba(0,0,0,.45) 45%, rgba(236,13,13,.14) 100%); pointer-events: none; }
    .play { width: 78px; height: 78px; border-radius: 50%; display: grid; place-items: center; border: 3px solid var(--paper); color: var(--paper); background: var(--red); box-shadow: 6px 6px 0 #000; font-family: 'Archivo Black', sans-serif; font-size: 1.7rem; padding-left: .2rem; position: relative; z-index: 2; transition: transform .2s ease; }
    .video-card:hover .play { transform: scale(1.08); }
    .video-caption { position: absolute; left: 1rem; right: 1rem; bottom: 1rem; display: flex; justify-content: space-between; gap: 1rem; align-items: end; border-top: 1px solid rgba(240,240,246,.25); padding-top: .8rem; z-index: 2; }
    .video-caption h3 { font-family: 'Archivo Black', sans-serif; text-transform: uppercase; font-size: clamp(1.4rem, 3vw, 3rem); line-height: .9; letter-spacing: -.06em; }
    .video-caption span { color: var(--red); text-transform: uppercase; font-size: .7rem; letter-spacing: .14em; white-space: nowrap; }
    #proyecto { background: linear-gradient(90deg, rgba(236,13,13,.16), transparent 42%), rgba(0,0,0,.18); border-block: 1px solid rgba(236,13,13,.24); }
    .project-grid { display: grid; grid-template-columns: .9fr 1.1fr; gap: clamp(1.5rem, 5vw, 4rem); align-items: start; margin-top: 2.5rem; }
    .project-cover { min-height: 520px; border: 2px solid var(--paper); background: linear-gradient(rgba(236,13,13,.15), rgba(0,0,0,.25)), linear-gradient(135deg, #4e2b36, #151319); box-shadow: 14px 14px 0 #000; position: sticky; top: 6rem; display: grid; place-items: center; text-align: center; padding: 0; overflow: hidden; }
    .project-cover-img { width: 100%; height: 100%; object-fit: cover; object-position: center; filter: contrast(1.08) saturate(0.95); transition: transform .35s ease, filter .35s ease; }
    ...
    .manifesto { padding: 1.2rem; border: 1px solid rgba(240,240,246,.16); background: rgba(240,240,246,.055); font-size: clamp(1rem, 1.7vw, 1.22rem); line-height: 1.75; color: rgba(240,240,246,.84); }
    .manifesto strong { color: var(--paper); }
    .services { display: grid; grid-template-columns: repeat(2, 1fr); gap: 1rem; margin-top: .5rem; }
    .service { display: block; padding: 1rem; border: 2px solid rgba(240,240,246,.65); background: rgba(0,0,0,.20); box-shadow: 5px 5px 0 #000; color: inherit; text-decoration: none; transition: transform .18s ease, box-shadow .18s ease, border-color .18s ease, background .18s ease; }
    .service:hover { transform: translate(3px, 3px); box-shadow: 2px 2px 0 #000; border-color: var(--red); background: rgba(236,13,13,.08); }
    .service h4 { font-family: 'Archivo Black', sans-serif; color: var(--red); text-transform: uppercase; font-size: 1.1rem; margin-bottom: .55rem; }
    .service p { color: rgba(240,240,246,.76); line-height: 1.55; font-size: .92rem; }
    .social-wall { display: grid; grid-template-columns: repeat(4, 1fr); gap: 1rem; margin-top: 2.5rem; }
    .social-card { min-height: 190px; padding: 1rem; border: 2px solid var(--paper); background: var(--red); box-shadow: 8px 8px 0 #000; display: flex; flex-direction: column; justify-content: space-between; transition: transform .18s ease, box-shadow .18s ease, background .18s ease; }
    ...
    .social-card strong { font-family: 'Archivo Black', sans-serif; font-size: clamp(1.8rem, 3vw, 3rem); text-transform: uppercase; line-height: .9; letter-spacing: -.06em; }
    footer { display: flex; justify-content: space-between; gap: 1rem; flex-wrap: wrap; padding: 2rem clamp(1rem, 4vw, 3rem); border-top: 1px solid var(--line); color: rgba(240,240,246,.64); font-size: .75rem; text-transform: uppercase; letter-spacing: .12em; }
...
<body>
  <div class="page">
    ...
    <section id="bio" class="section">
      <div class="bio-copy reveal">
        ...
        <div class="role-tags" aria-label="Roles y géneros">
      ...
      <aside class="bio-card reveal" aria-label="Foto principal">
        <img
    ...
    <section id="videos" class="section">
      ...
      <div class="video-layout reveal">
        <a class="video-card video-main"
           href="https://www.youtube.com/watch?v=YCxhXTU4NrE&list=RDYCxhXTU4NrE&start_radio=1"
           data-youtube-url="https://www.youtube.com/watch?v=YCxhXTU4NrE&list=RDYCxhXTU4NrE&start_radio=1"
           target="_blank" rel="noopener">
          <img src="" alt="" class="video-thumb" />
          <div class="play">&#9654;</div>
        ...
        <div class="video-stack">
          <a class="video-card"
             href="https://youtube.com/shorts/r7QvKXb2bo4?si=tNswV1ALOctfn68Q"
             data-youtube-url="https://youtube.com/shorts/r7QvKXb2bo4?si=tNswV1ALOctfn68Q"
             target="_blank" rel="noopener">
            <img src="" alt="" class="video-thumb" />
            <div class="play">&#9654;</div>
            <div class="video-caption">
          ...
          <a class="video-card"
             href="https://www.youtube.com/shorts/7lq5X5ncoKE"
             data-youtube-url="https://www.youtube.com/shorts/7lq5X5ncoKE"
             target="_blank" rel="noopener">
            <img src="" alt="" class="video-thumb" />
            <div class="play">&#9654;</div>
            <div class="video-caption">
  ...
  <script>
    ...
    // Miniaturas de YouTube (bug corregido: backticks faltantes en el original)
    ...
    document.querySelectorAll(".video-card[data-youtube-url]").forEach((card) => {
    ...
    // Auto-resize iframe para que Streamlit muestre todo el contenido
    function notifyHeight() {
      const h = document.documentElement.scrollHeight;
      window.parent.postMessage({ type: "streamlit:setFrameHeight", height: h }, "*");
      // Fallback directo sobre el iframe
      try {
        const frames = window.parent.document.querySelectorAll("iframe");
        frames.forEach((f) => { if (f.contentWindow === window) f.style.height = h + "px"; });
      } catch (e) {}
    }
